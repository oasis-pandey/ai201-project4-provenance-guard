...
Applications of AI Engineering Summer 2026 (@ Section 1C | Tuesday 5PM - 7PM PT)
Personal Member ID#: 150720
...
Show (Project)
 Report
📬 Reminder: Project 4 is due by Monday, June 29th at 1:59AM CDT.
...
## Progress
- Read and understood the project requirements, architecture guidelines, and milestone goals.
- Began setting up the `planning.md` template based on Milestone 2 specifications.

### planning.md
- Initialized the spec document template for Milestone 2.
- Defined **Signal 1 (Perplexity Score)** and **Signal 2 (Burstiness)**, detailing what they measure and their blind spots.
- Documented **Combination Logic** using min-max scaling and the weighted formula: `1.0 - (0.6 * P + 0.4 * B)`.
- Calibrated **Verdict Thresholds** (Human: `0 to 0.4`, Uncertain: `0.4 to 0.7`, AI: `0.7 to 1.0`) and defined the representation of a `0.6` confidence score.
- Wrote out final **Transparency Label** text variants for all three categories.
- Outlined the **Appeals Workflow** including eligibility, required fields, and reviewer dashboard information.
- Defined specific **Edge Cases** (structured poetry and common formal boilerplate).
- Added an **ASCII Architecture Diagram** and a detailed **Flow Narrative** for submission and appeal requests.
- Formulated the **AI Tool Plan** detailing the spec inputs, prompt directions, and verification steps for Milestones 3, 4, and 5.